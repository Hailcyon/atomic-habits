...
class EnvDes extends StatefulWidget {
  ...
  const EnvDes({Key? key, required this.onSave, required this.habitId}) : super(key: key);
...
class _EnvDes extends State<EnvDes> {
  ...
  void saveHabitLaw(BuildContext context, String habitStr) {
    ...
    if(habitStr.isEmpty) {
      ...
      String fin_habitStr = "Envrionment design:\n" + habitStr;

      // Save the habit and update the ID with the one generated by Firestore
      dbService.saveHabitLaw(widget.habitId, habitNum, habitLawNum, fin_habitStr);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Habit law has been added!"))
      );
